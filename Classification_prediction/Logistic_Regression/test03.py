...
# 标签频数统计
label_counts = df["car_acceptability"].value_counts()
print("标签频数统计：")
print(label_counts)
...
plt.title("Distribution of Car Acceptability Labels")
plt.axis('equal')  # 使饼图为圆形
plt.tight_layout()
plt.savefig('Distribution of Car Acceptability Labels.png')
plt.show()
...
# === 2. 定义模型与存储容器 ===
models = {
    "Logistic Regression": LogisticRegression(max_iter=1000, multi_class='multinomial', solver='lbfgs', penalty='l2',
                                              C=100.0),
