...
y_pred_4 = model_4.fit_predict(X)

model_best = AgglomerativeClustering(**best_params)
y_pred_best = model_best.fit_predict(X)
...
# 8. 构造伪“聚类中心”（使用特征均值）
centroids_k4 = pd.DataFrame(X.copy()).assign(cluster=y_pred_4).groupby("cluster").mean().reset_index(drop=True)
centroids_best = pd.DataFrame(X.copy()).assign(cluster=y_pred_best).groupby("cluster").mean().reset_index(drop=True)
...
def decode_centroids(df):
    decoded = []
    for _, row in df.iterrows():
        decoded_row = []
        for col in X.columns:
            val = int(round(row[col]))
            decoded_row.append(reverse_maps[col].get(val, "(?)"))
...
# 11. 打印聚类中心（未解码数值均值形式）
print("\n📌 K=4 的聚类中心（编码数值 - 特征均值）:")
print(centroids_k4.round(2).to_string(index=False))

print(f"\n📌 最佳K={best_k} 的聚类中心（编码数值 - 特征均值）:")
print(centroids_best.round(2).to_string(index=False))
...
plt.tight_layout()
plt.savefig("hierarchical_cluster_sample_distribution.png", dpi=300)
plt.show()
